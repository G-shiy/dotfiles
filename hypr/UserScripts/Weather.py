...
# status code
# Pega o texto do status
status_text = html_data("div[data-testid='wxPhrase']").text()

# Transforma em camelCase para usar como status_code
parts = status_text.strip().split()

status_code = parts[0].lower() + "".join(p.title() for p in parts[1:])
...
wind_speed = parts[1] if len(parts) > 1 else None
